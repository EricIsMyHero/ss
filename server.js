// server.js
// Quraşdırma: npm init -y
//           npm i express node-fetch
// Çalışdırma: TELEGRAM_BOT_TOKEN=xxx TELEGRAM_CHAT_ID=yyy node server.js
...
// Helper: real client IP (X-Forwarded-For dəstəyi)
function getClientIp(req) {
  const xff = req.headers['x-forwarded-for'];
  if (xff) return xff.split(',')[0].trim();
  // req.socket.remoteAddress ola bilər ::ffff:1.2.3.4 formatında
...
app.post('/collect', async (req, res) => {
  try {
    ...
    // 2) Telegram mesajı göndər (opt)
    if (TELEGRAM_BOT_TOKEN && TELEGRAM_CHAT_ID) {
      const text = `📣 Zarafat ziyarətçi!\nIP: ${ip}\nVaxt: ${time}\nUA: ${ua}\nSəhifə: ${page}`;
...
// Statik faylları servis et (index.html və s. eyni qovluqda olduqda)
app.use(express.static('public'));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`Server ${PORT}-da işə düşdü`));
